...
|---|---|---|
| `snd_syscall_resolve_ssn_scan` | `resolvers/scan.c` | Neighbor scan fallback when stub bytes are patched |
| `snd_syscall_resolve_ssn_sort` | `resolvers/sort.c` | Does not read stub bytes; derives SSN from export order |
...
```c
snd_status_t my_resolver(DWORD func_hash, snd_syscall_entry_t *entry_out) {
...
4. Find a function that allocates at least **120 bytes** and at most **496 bytes** (a capped "Fat Frame"). This ensures the legitimate function's shadow space is large enough to encapsulate our spoofed JMP-trampoline and its arguments without risking stack overflow. Note that these limits (`SND_SPOOF_FRAME_MIN_SIZE` and `SND_SPOOF_FRAME_MAX_SIZE`) are tightly coupled to the static stack allocation inside the ASM stub (e.g., `sub rsp, 512` in `spoofed_x64.asm`) and should not be changed without also updating the `.ALLOCSTACK` value in the assembly files to prevent stack corruption.
5. **Entropy Randomization:** To avoid creating a deterministic telemetry pattern (where every syscall spoofs the exact same function), the scanner uses an internal pseudo-randomizer. To ensure this is non-deterministic between runs, the seed is XOR'd with `KUSER_SHARED_DATA->SystemTime.LowPart` (mapped at the magic address `0x7FFE0014`), which injects system time entropy into the gadget selection. This guarantees it will skip a randomized number of valid Fat Frames before settling on one.
...
- Valid clean NTDLL base configured (via `snd_ntdll_set_clean`)
