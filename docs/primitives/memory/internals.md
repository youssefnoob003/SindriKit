...
Backends are composable across domains. A common stealth loader profile uses `snd_mem_sys` for allocations while using `snd_mod_nt` for import resolution — see `unified inject classic pe ... --sys`. Local NT-only loading without direct syscalls uses `snd_mem_nt` + `snd_mod_nt` as in `unified load pe ... --nt`.
