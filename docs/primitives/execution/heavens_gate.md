...
| Check | Status |
|---|---|
| `pFunctionAddress == 0` | `SND_STATUS_NULL_POINTER` |
| `dwArgCount > 6` | `SND_STATUS_TOO_MANY_ARGUMENTS` (with context string in debug builds) |
| `dwArgCount > 0 && pArgs == NULL` | `SND_STATUS_INVALID_PARAMETERS_COMBINATION` |
| `!snd_is_wow64()` | `SND_STATUS_ARCH_MISMATCH` |
...
From `pocs/src/cmd_hg.c` (invoked as `unified hg`):
