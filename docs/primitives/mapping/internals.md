...
> [!WARNING]
> `snd_map_sys` requires the operator to configure the syscall pipeline (`snd_ntdll_set_clean`, `snd_syscall_set_resolver`, `snd_syscall_set_invoker`) before invocation. Unconfigured pipelines cause immediate resolution failures.
