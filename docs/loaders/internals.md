...
**Prerequisite reading:** [PE parser techniques](../parsers/pe/internals.md), [COFF parser techniques](../parsers/coff/internals.md), [Dependency Injection](../architecture/dependency_injection.md)
...
Engine functions validate stage ordering and return `SND_STATUS_INVALID_STAGE` on violation.
...
After `snd_ldr_pe_execute_image`, the unified `load pe` command optionally resolves a named export and invokes it through the FFI bridge:
...
See [injection techniques](../injection/internals.md).
