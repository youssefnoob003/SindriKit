...
- Mapping backends — `snd_map_nt`, `snd_map_sys` — [mapping primitives](../primitives/mapping/internals.md)
...
|---|---|---|
| `pocs/src/cmd_load_pe.c` | `snd_ldr_pe_prepare_image`, `snd_ldr_pe_execute_image` | `--win`, `--nt`, or `--sys` backend |
| `pocs/src/cmd_load_coff.c` | `snd_ldr_coff_load`, `snd_ldr_coff_execute_image` | COFF/BOF execution |
...
- [internals.md](internals.md) — Pipeline, stage machine, protection models for PE & COFF
...
The repository PoC surface is a single `unified` executable. Use `unified load pe` or `unified load coff`; backend selection is a command-line concern, not a separate binary.
