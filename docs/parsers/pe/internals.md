...
### Robust RVA & Section Translation Guarantees

To ensure complete memory safety when processing untrusted or corrupted PE payloads, `snd_pe_rva_to_ptr` implements multiple strict safeguards:

- **Signed Reverse Section Iteration:** The reverse section scan uses a signed integer counter (`int i = (int)parser->sections_count - 1; i >= 0; i--`). This prevents unsigned integer underflow (`0U - 1U -> UINT_MAX`) when an RVA is not contained within any section.
- **Dynamic Header Boundary Clamping:** For header RVAs (`rva < size_of_headers`), `size_of_headers` is dynamically adjusted to `min(size_of_headers, min_raw_offset)` across all sections if section raw data starts before declared header size bounds.
- **Safe Overflow Arithmetic:** Pointer calculations and size alignments enforce overflow checks via helper macros (`SND_ADD_OVERFLOWS_DWORD`, `SND_RANGE_EXCEEDS`, `SND_IN_BOUNDS`, `SND_ALIGN_UP`, `SND_ALIGN_DOWN`).
- **Section Table Clamping:** During `snd_pe_parse`, `parser->sections_count` is automatically clamped to `max_sections = (available_space / sizeof(IMAGE_SECTION_HEADER))` to prevent section pointer reads from exceeding the buffer boundaries.
...
When `snd_pe_parse` is invoked with `is_mapped = TRUE` and `source.size == SND_SYS_DLL_SIZE_DEFAULT`, `snd_pe_parse` automatically expands `parser->source.size` to `OptionalHeader.SizeOfImage` if `SizeOfImage > 0`.

> [!NOTE]
> Automatic expansion ensures all subsequent directory parsing (imports, exports, relocations, TLS) has full coverage of the mapped image's address space without requiring manual buffer size adjustments after calling `snd_pe_parse`.
...
1. Bootstrap-parse the target image with `is_mapped = TRUE` (which automatically expands `parser->source.size` to `SizeOfImage` if initialized with `SND_SYS_DLL_SIZE_DEFAULT`).
2. Locate the export directory via `snd_pe_get_directory`.
3. Resolve by ordinal (`IS_INTRESOURCE`), name iteration, or hash comparison.
4. If the function RVA falls inside the export directory itself, treat it as a **forwarder**.
...
`snd_ldr_pe_execute_tls_callbacks` (in the PE loader engine) iterates the null-terminated callback pointer array and invokes each entry with the specified reason (`DLL_PROCESS_ATTACH`, etc.). Requires a mapped image and matching host/payload architecture.

---

## Internal Section Helpers

`section.h` exposes helpers used by the PE loader engine (`snd_pe_section_name`, `snd_pe_section_copy_size`, `snd_pe_section_loaded_size`). These helpers are part of the PE parser support surface and are consumed by `loaders/pe/engine.c`.
