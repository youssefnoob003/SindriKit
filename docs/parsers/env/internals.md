...
## NTDLL State Encapsulation

The `parsers/env/ntdll.h` module encapsulates NTDLL location and parsing state to centralize syscall resolution. Instead of each backend manually locating NTDLL and parsing its exports, the state is cached globally:

- **Active NTDLL:** Lazily initialized on first use by scanning the local PEB. Used for `Nt*` procedure location during syscall indirect invocation (via `snd_ntdll_get_active_export`).
- **Clean NTDLL:** Explicitly configured via `snd_ntdll_set_clean(base)` (usually after mapping a fresh copy from `\KnownDlls\`). Used by SSN resolvers to extract unaltered syscall numbers (via `snd_ntdll_get_clean_export`).

This separation ensures that SSN extraction relies on a clean, unhooked image while execution (e.g. gadget finding) relies on the active, executable image.
...
- **Null checks:** Both public functions validate `out_base` and return `SND_STATUS_NULL_POINTER` on bad input. A missing module returns `SND_STATUS_MODULE_NOT_FOUND`.
