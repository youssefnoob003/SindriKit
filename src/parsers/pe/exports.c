...
static snd_status_t pe_get_export_address_impl(PVOID base_address, SIZE_T size, const char *func_name, DWORD func_hash,
    ...
    *func_addr_out = NULL;
    ...
    snd_status_t    status = snd_pe_parse(&local_buf, TRUE, &local_parser);
    ...
    if (func_rva >= export_dir.VirtualAddress && func_rva < export_dir.VirtualAddress + export_dir.Size) {
        ...
        PVOID fwd_base = NULL;
        status         = resolver(wfwd_dll, &fwd_base);
        if (SND_FAILED(status) || !fwd_base)
        ...
        if (*fwd_func == '#') {
            ...
            return pe_get_export_address_impl(fwd_base, SND_SYS_DLL_SIZE_DEFAULT, (const char *)ord, 0, func_addr_out,
                                              resolver, FALSE, depth + 1);
        }
...
snd_status_t snd_pe_get_export_address(PVOID base_address, SIZE_T size, const char *func_name, FARPROC *func_addr_out,
    ...
    return pe_get_export_address_impl(base_address, size, func_name, 0, func_addr_out, resolver, FALSE, 0);
}

snd_status_t snd_pe_get_export_address_hash(PVOID base_address, SIZE_T size, DWORD func_hash, FARPROC *func_addr_out,
                                            snd_module_resolver_cb resolver) {
    return pe_get_export_address_impl(base_address, size, NULL, func_hash, func_addr_out, resolver, TRUE, 0);
}
