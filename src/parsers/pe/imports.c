...
snd_status_t snd_pe_resolve_imports(PVOID base_address, const snd_module_api_t *mod_api,
    ...
    if (!parser->is_mapped) {
        return SND_ERR_CTX(SND_STATUS_IMAGE_NOT_MAPPED, "Imports can only be resolved on mapped images.");
    }
    ...
    while (1) {
        ...
        HMODULE      h_dll  = NULL;
        snd_status_t status = mod_api->load_library(dll_name, &h_dll);
        if (SND_FAILED(status) || !h_dll) {
        ...
        for (; resolved < max_thunks; resolved++) {
            ...
            if (is_ordinal) {
                ...
                status            = mod_api->get_proc_address(h_dll, (LPCSTR)ordinal, &func_addr);
                ...
                status = mod_api->get_proc_address(h_dll, (LPCSTR)func->Name, &func_addr);
                if (SND_FAILED(status)) {
                    return status;
                }
            }
