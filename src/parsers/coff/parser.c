...
snd_status_t snd_coff_parse(const snd_buffer_t *buf, snd_coff_parser_t *parser) {
    SND_CHECK_NULL(buf, buf->data, buf->size, parser);

    snd_memzero(parser, sizeof(snd_coff_parser_t));
    parser->source = *buf;

    if (!snd_buffer_bounds_check(buf, 0, sizeof(SND_IMAGE_FILE_HEADER))) {
        return SND_ERR(SND_STATUS_HEADER_FILE_TRUNCATED);
    }

    parser->file_header = (PSND_IMAGE_FILE_HEADER)buf->data;

    if (parser->file_header->Machine == SND_IMAGE_FILE_MACHINE_AMD64) {
        parser->is_64bit = TRUE;
    } else if (parser->file_header->Machine == SND_IMAGE_FILE_MACHINE_I386) {
        parser->is_64bit = FALSE;
    } else {
        return SND_ERR(SND_STATUS_HEADER_MACHINE_UNSUPPORTED);
    }

    parser->sections_count = parser->file_header->NumberOfSections;

    SIZE_T sections_off = sizeof(SND_IMAGE_FILE_HEADER) + parser->file_header->SizeOfOptionalHeader;

    if (parser->sections_count > 0) {
        SIZE_T sections_size = (SIZE_T)parser->sections_count * sizeof(SND_IMAGE_SECTION_HEADER);

        if (!snd_buffer_bounds_check(buf, sections_off, sections_size)) {
            return SND_ERR(SND_STATUS_SECTION_TABLE_TRUNCATED);
        }
        parser->section_head = (PSND_IMAGE_SECTION_HEADER)SND_PTR_ADD(buf->data, sections_off);
    }
    ...
    if (symbol_table_off != 0 && parser->symbol_count > 0) {
        if (SND_MUL_OVERFLOWS_SIZET(parser->symbol_count, sizeof(SND_IMAGE_SYMBOL))) {
            return SND_ERR(SND_STATUS_SYMBOL_TABLE_OVERFLOW);
        }

        SIZE_T symbols_size = (SIZE_T)parser->symbol_count * sizeof(SND_IMAGE_SYMBOL);

        if (!snd_buffer_bounds_check(buf, symbol_table_off, symbols_size)) {
            return SND_ERR(SND_STATUS_SYMBOL_TABLE_TRUNCATED);
        }
        parser->symbol_table = (PSND_IMAGE_SYMBOL)SND_PTR_ADD(buf->data, symbol_table_off);

        if (SND_ADD_OVERFLOWS_SIZET(symbol_table_off, symbols_size)) {
            return SND_ERR(SND_STATUS_SYMBOL_TABLE_OVERFLOW);
        }
