...
static snd_status_t WINAPI sys_create_process(const wchar_t *image_path, const wchar_t *command_line,
    ...
    // so we fall back to the Win32 subsystem for process creation even in the SYS backend.
...
const snd_process_api_t snd_proc_sys = {.create_process       = sys_create_process,
