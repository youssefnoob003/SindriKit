...
PUBLIC snd_invoke_syscall_asm

; snd_invoke_syscall_asm(SND_SYSCALL_ARGS* args)
snd_invoke_syscall_asm PROC
    ; Allocate 88 bytes on stack to safely store args 5-10
    ; (32 bytes shadow space + 48 bytes for 6 args + 8 bytes padding for 16-byte alignment)
    sub rsp, 88
    
    ; RCX points to SND_SYSCALL_ARGS struct
    ; Setup stack arguments (args 5-10) safely in our new frame
    mov r11, [rcx+40]   ; arg5
    mov [rsp+40], r11
    mov r11, [rcx+48]   ; arg6
    mov [rsp+48], r11
