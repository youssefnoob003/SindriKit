...
snd_status_t snd_syscall_add_resolver(snd_syscall_resolver_t resolver) {
    SND_CHECK_NULL(resolver);
    if (g_strategy_count < SND_MAX_INTERNAL_STRATEGIES) {
        g_strategy_chain[g_strategy_count++] = resolver;
        return SND_OK;
    }

    return SND_ERR_CTX(SND_STATUS_SYSCALL_PIPELINE_EXHAUSTED, "Maximum registered syscall strategies (%d) exceeded",
                       SND_MAX_INTERNAL_STRATEGIES);
}
...
snd_status_t snd_syscall_resolve(DWORD func_hash, snd_syscall_entry_t *entry_out) {
    ...
    SND_TRY(snd_ntdll_get_clean_base(&clean_ntdll));
    ...
    for (int i = 0; i < g_strategy_count; i++) {
        ...
        snd_status_t status = g_strategy_chain[i](func_hash, entry_out);
...
snd_status_t snd_syscall_invoke(DWORD sys_hash, snd_syscall_args_t *args, NTSTATUS *out_nt_status) {
    ...
    SND_TRY(snd_syscall_resolve(sys_hash, &entry));
    ...
    *out_nt_status = g_syscall_invoker(args);
    return SND_OK;
}
