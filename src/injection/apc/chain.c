...
snd_status_t snd_inj_apc_shell(snd_inj_ctx_t *ctx) {
    ...
    SND_INJ_TRY(ctx, snd_inj_apc_execute(ctx));
    return SND_OK;
}
...
snd_status_t snd_inj_apc_pe(snd_ldr_pe_ctx_t *ldr_ctx, snd_inj_ctx_t *inj_ctx) {
    ...
    SND_INJ_TRY_WITH_CLEANUP(inj_ctx, snd_ldr_pe_free_mapped_image(ldr_ctx), snd_inj_apc_execute(inj_ctx));
    return SND_OK;
}
...
snd_status_t snd_inj_apc_coff(snd_ldr_coff_ctx_t *ldr_ctx, snd_inj_ctx_t *inj_ctx, const char *entry_point, void *args,
    ...
    SND_INJ_TRY_WITH_CLEANUP(inj_ctx, snd_ldr_coff_free_mapped_image(ldr_ctx), snd_inj_apc_execute(inj_ctx));
    return SND_OK;
}
